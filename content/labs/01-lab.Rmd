...
You can either download the lab as an RMarkdown file <a href="../../labs/01-lab.Rmd" download="01-lab.Rmd">here</a>, or copy and paste the code as we go into a `.R` script. Either way, save it into the `01-week` folder where you completed the exercises!
...
5. Navigate between previously run lines of code in the console with the up and down arrows. Add `cmd/ctrl` to look at the entire list.

6. Search for files and functions with `cmd/ctrl` + `.`.
  - Look for and open the `01-todo.R` file.

7. Autocomplete snippets... try it out by typing `lib` then `enter` once the autocomplete pops up.
```{r}
# write library(tidyverse) using the snippet here:

```

8. As elsewhere, `cmd/ctrl` + `f` is find, but with `cmd/ctrl` + `shift` + `f` you can find across all the files in a directory.
  - Look for the word "basics" throughout the directory

9. Drag a cursor to extend across multiple lines with `opt/alt`.
```{r}
...
10. Put a new cursor elsewhere by holding down `opt/alt` + `shift` as you click.
```{r}
# edit all these numbers to be in the 200s, not 100s
numbers <- c(100, 101, 104, 109)
```

11. Reformat code with `cmd/ctrl` + `shift` + `a`.
```{r}
# try it out here:
mat <- matrix(c(
...
```{r}
library(tidyverse)
nlsy <- read_csv("https://raw.githubusercontent.com/louisahsmith/intro-to-R/master/data/nlsy_cc.csv",
                 col_names = c("glasses", "eyesight", "sleep_wkdy", "sleep_wknd",
                               "id", "nsibs", "samp", "race_eth", "sex", "region", 
                               "income", "res_1980", "res_2002", "age_bir"), 
                 skip = 1) # this means to skip the first row, which are the NLSY-assigned column names
```
...
download.file("https://github.com/louisahsmith/intro-to-R/blob/master/data/nlsy_cc.xlsx?raw=true",
