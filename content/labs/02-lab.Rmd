...
You can either download the lab as an RMarkdown file <a href="../../lab-handouts/02-lab.Rmd" download="02-lab.Rmd">here</a>, or copy and paste the code as we go into a `.R` script. Either way, save it into the `02-week` folder where you completed the exercises!
...
We used the `shape = ` argument to map the sex variable to different shapes. We wanted to change the shapes to squares and diamonds. Let's explore how we might go about this. Here are three things we might try. See if you can spot why error messages appear for the first two.
